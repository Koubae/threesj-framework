...
function getCollider( geometry ) {
	...
	if ( geometry.type === 'BoxGeometry' ) {
		...
		return Physics.ColliderDesc.cuboid( sx, sy, sz );

	} else if ( geometry.type === 'SphereGeometry' || geometry.type === 'IcosahedronGeometry' ) {

		const radius = parameters.radius !== undefined ? parameters.radius : 1;
		return Physics.ColliderDesc.ball( radius );
...
async function RapierPhysics() {
	...
	function createBody( position, quaternion, mass, shape ) {
		...
		const desc = mass > 0 ? Physics.RigidBodyDesc.dynamic() : Physics.RigidBodyDesc.fixed();
